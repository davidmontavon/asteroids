$(document).ready(function(){
    ...
    var vr = 0.0;
    var dead = false;
    var containerWidth = 900.0;
    var containerHeight = 650.0;
    var keyUpIsDown = false;
    var keyLeftIsDown = false;
    var keyRightIsDown = false;
    var keySpaceIsDown = false;
    var nbTicks = 0;
    var bulletV = 30.0;
    var asteroidVMax = 8.0;
    var asteroids = [];
    var bullets = [];
    var explosionSparks = [];
    ...
    $("#soundEffectEnabled").change(function(){
      soundEffectsEnabled = $(this).is(":checked");
    });
    ...
    function checkScore(callback){
      ...
      var newScoreEntry = function(){ return { name: prompt("Enter your name:"), score: score } };
    ...
    function init(){
        ...
        canvasFX.width = containerWidth;
        canvasFX.height = containerHeight;
    
        $("#gameState").css("width", containerWidth);
        $("#score").html(score);    
        $("#gameOverScreen").css("width", containerWidth);
        $("#gameOverScreen").css("height", containerHeight);
        $("#topScoresScreen").css("width", containerWidth);
        $("#topScoresScreen").css("height", containerHeight);
    
        x = containerWidth / 2.0 - 30.0;
        y = containerHeight / 2.0 - 30.0;
    ...
    function middleOfPlayFieldIsFree(){
        var middleX = containerWidth / 2.0;
        var middleY = containerHeight / 2.0;
        var radius = 100.0;
        var ret = true;
    ...
    function refreshAsteroids(){
      asteroids.map(asteroid => {
        ...
        if (asteroid.x + halfRadius > containerWidth)
          asteroid.x = -halfRadius;
        else if (asteroid.x + halfRadius < 0)
          asteroid.x = containerWidth - halfRadius;
        if (asteroid.y + halfRadius > containerHeight)
          asteroid.y = -halfRadius;
        else if (asteroid.y + halfRadius < 0)
          asteroid.y = containerHeight - halfRadius;
    ...
    */
    ...
    function ticks(){
        ctxFX.clearRect(0, 0, containerWidth, containerHeight);
        ...
        if (!dead){
            ...
            if (x + 100 > containerWidth)
                x = -100;
            else if (x + 100 < 0)
                x = containerWidth - 100;

            if (y + 30 > containerHeight)
                y = -30;
            else if (y + 30 < 0)
